...
BUD-01 후보의 현재 잔여 판단은 평가 시점까지 관측된 현금, 실제 사용 후 남은 계획, 별도 미집행 예약 및 미납 카드대금으로 계산한다. 월초 원안과 미래 예정 입금을 더한 예상 잔여는 현재의 자유 현금과 별도로 표시한다. 이미 지난 예정 입금이나 목표 납입은 관측 기록 없이 실제 사실로 바꾸지 않는다. 목표 배정 후 날짜별 가용현금은 총현금 경로와 구분하고, 보호액·목표 배정의 중복 차감을 피한다. 구체적 필드와 계산 순서는 [ARCHITECTURE](docs/ARCHITECTURE.md#개인-재무와-투자-원장의-경계--향후-설계)에 둔다.
