...
| OD-19 | WEB-01 로컬 예산 미리보기 후보의 웹 adapter는 FastAPI 요청 경계, Jinja2/기본 HTML·CSS 화면, Uvicorn 루프백 실행을 사용한다. 재무 자료를 저장하지 않고 기존 순수 계산기로 전달한다. 인증, 모바일 원격접속, 외부 공개, 복구, 텔레그램 허용 조회 범위와 운영 개인정보 보호는 미결정·후속 검증이다 | WEB-01 후보 리뷰; 원격·운영 연결 전 별도 결정 |
