...
하나의 플랫폼에서 다음 세 영역을 연결한다. 개인 예산은 제품 범위에 추가된 요구사항이다. BUD-01의 오프라인 계산기는 main에 구현됐으며 저장·실적 비교·실제 자금 연결은 후속 단계다.
...
BUD-01의 `existing_protected`는 `balance_date`의 현금 잔액과 같은 기준일에 관측한 보호총액 스냅샷이다. `GoalContribution`은 자유자금에서 새로 보호한 금액만 나타내며, 스냅샷에 이미 포함된 납입은 명시적으로 연결한다. `protected_floor`는 그 보호총액의 하한이며 별도 목적의 추가 보호액이 아니다. 보호 목적만 변경한 거래나 포함 관계를 알 수 없는 입력은 계산하지 않는다.

BUD-01 구현의 현재 잔여 판단은 평가 시점까지 관측된 현금, 실제 사용 후 남은 계획, 별도 미집행 예약 및 미납 카드대금으로 계산한다. 확인된 카드 사용의 미결제 의무는 이번 달 납부 일정이 없어도 금액을 보존하며, 지급 예정과 실제 결제를 구분한다. 월초 원안과 미래 예정 입금을 더한 예상 잔여는 현재의 자유 현금과 별도로 표시한다. 이미 지난 예정 입금이나 목표 납입은 관측 기록 없이 실제 사실로 바꾸지 않는다. 목표 배정 후 날짜별 가용현금은 총현금 경로와 구분하고, 보호액·목표 배정의 중복 차감을 피한다. 현재 투자 제안은 평가일 이후의 자금 조건으로 판단하고 과거 부족은 이력으로 보존한다. 완료돼 불필요한 목표 자금 검사는 확인 불가와 구별한다. 구체적 필드와 계산 순서는 [ARCHITECTURE](docs/ARCHITECTURE.md#개인-재무와-투자-원장의-경계--향후-설계)에 둔다.
...
| OD-19 | main에 채택된 WEB-01 로컬 예산 미리보기의 웹 adapter는 FastAPI 요청 경계, Jinja2/기본 HTML·CSS 화면, Uvicorn 루프백 실행을 사용한다. 재무 자료를 저장하지 않고 기존 순수 계산기로 전달한다. 인증, 모바일 원격접속, 외부 공개, 복구, 텔레그램 허용 조회 범위와 운영 개인정보 보호는 미결정·후속 검증이다 | 원격·운영 연결 전 별도 결정; WEB-01 병합은 운영 배포 승인 아님 |
