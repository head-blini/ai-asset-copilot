...
from asset_copilot.portfolio.calculator import _div
...
def config(**changes):
    values = dict(policy_version="test-v1", single_asset_warn_ratio=D("0.30"),
                  single_asset_breach_ratio=D("0.40"), direct_sector_warn_ratio=D("0.30"),
                  direct_sector_breach_ratio=D("0.40"), min_cash_ratio=D("0.05"),
                  max_cash_ratio=D("0.30"))
...
def analysis(*, total="100", cash="20", positions=(), sectors=(), unclassified="0"):
    ...
    return PortfolioAnalysis(
        ...
        stock_exposure=None, etf_exposure=None, usd_exposure=D("1") if total else None,
...
def test_config_is_immutable_and_existing_toml_is_loaded():
    ...
    assert configured == config(
        policy_version="initial-v1", single_asset_warn_ratio=D("0.12"),
        single_asset_breach_ratio=D("0.15"), direct_sector_warn_ratio=None,
...
def test_single_asset_thresholds_and_equality(value, status, code):
    item = position("asset-a", value, "100")
    result = PortfolioPolicyEngine(config()).evaluate(analysis(cash="20", positions=(item,)))
    actual = decision(result, PolicyKind.SINGLE_ASSET)
...
def test_all_direct_assets_are_evaluated_independently_and_ordered():
    ...
    assets = [item for item in result.evaluations if item.policy is PolicyKind.SINGLE_ASSET]
    assert [(item.target.identifier, item.status) for item in assets] == [
        ("a", PolicyStatus.WARN), ("m", PolicyStatus.PASS), ("z", PolicyStatus.BREACH),
    ]
...
def test_cash_is_in_denominator_and_display_rounding_does_not_decide():
    ...
    assert decision(result, PolicyKind.SINGLE_ASSET).status is PolicyStatus.WARN
    assert decision(result, PolicyKind.SINGLE_ASSET).actual_ratio == D("0.3999")
...
def test_repeating_ratio_comparison_is_context_independent():
    ...
    configured = config(single_asset_warn_ratio=threshold,
                        single_asset_breach_ratio=threshold)
    ...
    assert decision(ordinary, PolicyKind.SINGLE_ASSET).status is PolicyStatus.BREACH
...
def test_zero_value_and_no_applicable_positions_are_explicit():
    ...
    assert result.not_applicable == (PolicyKind.SINGLE_ASSET, PolicyKind.DIRECT_SECTOR)
    item = decision(result, PolicyKind.CASH_RATIO)
    assert item.status is PolicyStatus.UNKNOWN
...
def test_cash_only_and_etf_only_have_no_direct_sector_evaluation():
    ...
    assert cash_only.not_applicable == (PolicyKind.SINGLE_ASSET, PolicyKind.DIRECT_SECTOR)
    ...
    assert etf_only.not_applicable == (PolicyKind.DIRECT_SECTOR,)
    assert decision(etf_only, PolicyKind.SINGLE_ASSET).status is PolicyStatus.BREACH
...
def test_missing_exposure_is_unknown_and_input_and_report_are_immutable():
    ...
    assert data == before
    assert decision(report, PolicyKind.SINGLE_ASSET).status is PolicyStatus.UNKNOWN
    with pytest.raises(FrozenInstanceError):
...
def test_same_analysis_and_config_produce_identical_report_without_clock():
    ...
    assert [item.status for item in first.evaluations] == [
        PolicyStatus.BREACH, PolicyStatus.BREACH, PolicyStatus.PASS,
    ]
...
def test_real_analyzer_output_preserves_direct_stock_scope_and_needs_no_io_during_policy(tmp_path):
    with SQLiteStore(tmp_path / "policy.sqlite") as store:
        ...
        report = PortfolioPolicyEngine(config()).evaluate(result)
        ...
        assert decision(report, PolicyKind.SINGLE_ASSET, identifier="etf").actual_ratio == D("0.5")
...
def test_stale_quote_is_rejected_before_policy_analysis_exists(tmp_path):
    with SQLiteStore(tmp_path / "empty.sqlite") as store:
        ...
        assert report.not_applicable == (PolicyKind.SINGLE_ASSET, PolicyKind.DIRECT_SECTOR)
