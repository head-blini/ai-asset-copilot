...
이 문서는 [PROJECT_SPEC.md](../PROJECT_SPEC.md)의 요구사항을 설계로 설명한다. **main의 현재 구현은 Portfolio Foundation, Market Data 정규화, 현재 미국 계좌 분석, BUD-01 오프라인 계산기와 WEB-01 루프백 미리보기다.** `feat/phase-4-policy-engine`과 후속 P4-05 Policy evaluator는 미병합 후보이며 현재 main의 기능이 아니다. Budget/Cashflow의 순수 계산·오프라인 보고와 로컬 웹 adapter는 main에 병합됐다. AI·Risk·Execution·Broker·원격 웹·운영 흐름은 향후 설계다. main의 정책 설정 파일은 아직 실행 코드에서 읽지 않는다.
...
| `web` | 루프백 HTML 입력·검증·표시 adapter | `BudgetInput` 생성 후 기존 `calculate_month()` 호출; 재무 자료 저장 없음 |
...
`BudgetInput.existing_protected`는 `cash_balance`와 같은 `balance_date` 기준의 보호총액 스냅샷이다. 현금 잔액은 그날 `CashEntry`를 재생하기 직전의 값이며, 같은 날의 목표 납입이 보호 스냅샷에 포함됐는지는 별도 명시한다. 각 `GoalContribution.protection_source`는 `FREE_CASH`로 명시해야 한다. 이는 자유자금에서 새로 보호한 배정 기록이며 현금 이체 기록은 아니다. 이미 보호된 돈의 목표만 바꾼 납입과 미지정 출처는 BUD-01에서 거부한다. `protected_contributions_in_balance`는 `(goal_id, contribution day)` 목록으로, 스냅샷에 이미 포함된 이번 달 납입만 선언한다. 스냅샷 이전 날짜의 납입은 모두 목록에 있어야 하며, 같은 날짜의 납입은 포함 또는 미포함을 명시한 목록으로 구별한다. 키 중복·미존재·미래 날짜, 포함 금액이 스냅샷 보호액을 넘는 경우는 거부한다. 포함 관계가 필요한데 목록이 없으면 추정하지 않는다.

보호총액은 날짜마다 `max(existing_protected + 스냅샷에 없는 그날까지의 신규 목표 납입, protected_floor)`로 계산한다. `protected_floor`는 **같은 보호총액의 하한**이고 별도 가산액이 아니다. 현재 margin, 목표 납입일의 자금 검사, `cash_days.available_cash`가 이 규칙을 공유한다. 목표의 `NOT_REQUIRED`는 납입 완료로 미래 검사가 불필요하다는 뜻일 뿐, 이미 보호된 납입을 자유자금으로 돌리지 않는다. `saved_amount`는 목표 진척에 사용하며 보호 스냅샷 포함 여부를 대신하지 않는다.
...
현재의 미집행 예약은 시작일 이력이 없으므로 평가일 이전의 `cash_days.available_cash`에 소급하지 않는다. 과거 현금 부족은 날짜와 사유로 남기되 현재 투자 제안의 일별 조건에는 평가일과 이후만 사용한다. 같은 현재 현금·의무·정책이면 월초 재생과 월중 관측 잔액에서 같은 현재 배정 판단을 낸다. 목표별 `funding_check_status`는 날짜별 검사를 수행한 `CHECKED`, 목표액 달성 또는 이번 달 납입 관측으로 검사가 불필요한 `NOT_REQUIRED`, 필요한 사실이나 정책이 없어 검사할 수 없는 `UNKNOWN`을 구분한다. 과거 납입이 부분 관측 또는 미확인이고 목표 잔여액이 양수면 납입 완료 관측이나 수정 일정을 요청하며 투자 제안을 보류한다. `NOT_REQUIRED`여도 이미 관측된 목표저축과 명시된 목표 배정은 보호 상태를 유지한다.
