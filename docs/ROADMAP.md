...
| --- | --- | --- |
| BUD-01 | 개인 1명/KRW/월의 수동 가상 입력을 받는 순수 예산 계산기와 로컬 실행 예제·사람이 읽을 수 있는 보고 | 명시적 사용자 정책으로 실수령 소득, 개시 현금, 날짜별 지출·상환, 목표·적립액 및 기존 사용·예약액을 검증한다. 생활비·자유지출·필수 상환·비상자금·목표저축·미국 장기투자·한국 전략용 자금의 월 배정안, 미배정액·부족액·원인, 날짜별 현금 부족을 반환한다. 필수 입력·정책 누락은 0으로 채우지 않고 필요한 정보를 명시한다. 한 번의 로컬 명령으로 가상 입력의 보고를 출력하고 테스트가 통과해야 한다. 은행·카드·Toss·LLM·주문 연결 없음. P4-06/07 완료를 선행 조건으로 두지 않는다 |
...
- **목적/입력:** 한 사람의 한 달에 대해 수동 작성한 가상 KRW 실수령 입금(실제/확정 예정/불확실 구분), 개시 가용현금과 기존 보호 잔액, 날짜별 고정비·생활비·자유지출·카드 사용/납부·부채 원금/이자·계좌 간 이동, 목표 금액/기한/우선순위/기존 적립·이번 적립, 이미 지출·예약한 금액, 사용자 소유 정책, 기준 월/평가 시점을 받는다. 세전액은 참고 정보로만 사용하고 실수령액에서 기존 공제를 또 빼지 않는다. 실제 정책 값이 없으면 미설정으로 남긴다. 가상 예제 숫자는 운영 기본값이 아니다.
- **계산/출력:** 명시적 순서의 규칙으로 월초 배정안과 월중 잔여안을 구별하고, 일자별 입금·납부를 반영한 현금 궤적, 각 용도별 배정·이미 사용·잔여, 미배정액, 부족액과 그 원인, 목표 기한까지 필요한 적립액과 납입 가능일, 판단 불가 항목과 필요한 입력을 반환한다. 배정은 실제 이체가 아니다. 월말 합계가 흑자여도 월중 부족이면 부족 날짜와 금액을 표시한다. 미래 투자수익·불확실 소득·대출·미실현 이익으로 적자를 숨기지 않는다. `Decimal`·통화·출처·시각을 유지하고 입력 순서에 의존하지 않는 결과를 낸다.
