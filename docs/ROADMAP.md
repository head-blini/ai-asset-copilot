...
### WEB-01 로컬 월간 예산 미리보기

| 단위 | 범위 | 리뷰 완료 조건 |
| --- | --- | --- |
| WEB-01 | 루프백 전용 FastAPI/Jinja2 입력·결과 화면. 기존 BUD-01 `BudgetInput`·`calculate_month()` 사용. 직접 입력과 명시적으로 불러오는 일곱 가상 예제, 수정·재계산. | 입력 누락·0·UNKNOWN, 카드/예약/보호 관계를 보존하고 월간 결과를 분리 표시한다. Host·Origin·CSRF, 입력 크기, no-store와 무저장 경계를 검증한다. core와 웹 CI, 일반 패키지 설치, 실제 브라우저 PC·모바일 폭 경로를 확인한 뒤 리뷰한다. main 병합·운영 배포는 별도 상태다. |

WEB-01은 예산 계산기의 로컬 사용자 경로이며 M1의 실제 미국 계좌 입력·대사·Policy 보고 경로와 다르다. Phase 11 전체 Dashboard/Daily/Weekly Report의 범위·완료 조건을 줄이거나 완료 처리하지 않는다. 인증·휴대폰 원격접속·복구·텔레그램·실제 자금 연결은 이 단위의 완료 조건이 아니다.

## Budget / Cashflow Planning 확장

2026-09-25 사용자 요청으로 개인 현금흐름·예산·목표저축을 제품 범위에 추가했다. **main에는 BUD-01 오프라인 계산기가 구현됐다.** BUD-02~04는 아직 구현되지 않았다. 이 BUD 작업은 Phase 0–13의 번호와 각 완료 조건을 변경하지 않는다. 기존 M1의 미국 계좌 입력→대사→평가→Policy→최소 보고는 유지한다. BUD-04에서 그 검증된 미국 자산 상태와 개인 예산을 함께 보고하되, BUD-01의 순수 계산은 M1이나 P4-06/07의 완료에 불필요하게 종속시키지 않는다. 반대로 실제 투자 자금 연결인 BUD-03은 승인된 예산, 가용현금, 입력·대사 및 기존 Policy/Risk 경계가 검증된 뒤에만 진행한다. P4-05는 별도 미병합 후보이며 BUD-01 채택으로 Phase 4 또는 M1 완료 판정을 앞당기지 않는다. Phase 5–13과 그 gate는 그대로 유지한다.
...
- **비범위/산출물:** 순수 계산 코드, 집중된 단위 테스트, 한 번 실행해 사람이 읽을 수 있는 가상 월간 보고를 출력하는 오프라인 예제와 기존 문서의 사용 안내가 산출물이다. DB, 은행/카드 API, Toss, 세금 계산, 대출 최적화, 다중통화 예산, 실시간 결제 차단, LLM, 실제 송금·환전·주문은 넣지 않는다. 구현·테스트·오프라인 보고의 채택 증거는 [STATUS](STATUS.md)에 기록한다. 문서 변경만으로 BUD-01을 완료 처리하지 않는다.
...
BUD-01의 순수 계산·가상 입력·정상/급여 지연/목표 미정 및 회귀 시나리오 보고는 검토·검증 후 main에 병합됐다. 결과는 승인·이체·주문이나 BUD-02/03 구현이 아니다. 테스트·CI·병합 증거는 [STATUS](STATUS.md)에 기록하고, 기존 완료 조건은 유지한다.
